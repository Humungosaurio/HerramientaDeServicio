...
const AsistenciasPersonal = () => {
  ...
  const cargarAsistencias = async () => {
    if (window.pywebview && window.pywebview.api && window.pywebview.api.cargar_matriz_asistencia_personal) {
      try {
        const payload = {
        ...
        const res = await window.pywebview.api.cargar_matriz_asistencia_personal(payload);

        if (res.status === "success") {
          const datosAdaptados = res.data.map((emp) => {
            const asistenciaVisual = {};
            diasSemana.forEach((dia) => {
              const val = emp.asistencia?.[dia];
              asistenciaVisual[dia] = (val !== undefined && val !== null && val !== "" && val !== 0 && val !== "0.0") ? val : "";
            });
            return {
              ...emp,
        ...
        console.error("❌ Error crítico de comunicación con Python:", error);
      }
    } else {
      console.warn("⚠️ Entorno de pywebview no detectado. No se pudieron cargar los datos.");
    }
  };

  // Escuchamos cambios en los filtros y aseguramos que pywebview esté listo
  useEffect(() => {
    ...
    if (window.pywebview) {
      ...
      window.addEventListener("pywebviewready", intentarCargar);
    }

    return () => {
      window.removeEventListener("pywebviewready", intentarCargar);
    };
  }, [turnoSeleccionado, mesSeleccionado, semanaSeleccionada]);
  // ✅ CORREGIDO: Se eliminó el "};" extra que rompía la compilación aquí.

  // 3. FUNCIONES DE CÁLCULO Y EDICIÓN DE TABLA (AGREGADAS)
  ...
  const actualizarHorasDia = (cedulaEmp, dia, valor) => {
    setPersonal((prevPersonal) =>
      prevPersonal.map((emp) => {
        if (emp.cedula === cedulaEmp) {
          return {
            ...emp,
            asistencia: {
              ...emp.asistencia,
  ...
  // 4. GESTIÓN DE ESTADOS DEL PERSONAL (AGREGADA)
  const cambiarEstadoPersonal = async (cedulaEmp, nuevoEstado) => {
    // Actualización local para una respuesta instantánea en la UI
    setPersonal((prevPersonal) =>
      prevPersonal.map((emp) =>
        emp.cedula === cedulaEmp ? { ...emp, estado: nuevoEstado } : emp
      )
    );

    // Si tu backend en Python tiene un endpoint para esto, se comunica aquí:
    if (window.pywebview && window.pywebview.api && window.pywebview.api.actualizar_estado_trabajador) {
      try {
        await window.pywebview.api.actualizar_estado_trabajador({
          cedula: cedulaEmp,
          estado: nuevoEstado,
        });
      } catch (error) {
        console.error("❌ Error al cambiar estado en Python:", error);
      }
    }
  };
...
const guardarCambios = async () => {
    const registrosLimpios = personal.map((empleado) => {
      ...
      return {
        ...
        estado: empleado.estado // ✨ LÍNEA NUEVA: Agregamos el estado al paquete
      };
    });
  ...
  const personalAProcesar = [...personal]
    .filter((emp) =>
      vistaActual === "administracion" ? true : emp.estado === "Activo"
    )
    .sort((a, b) => {
      if (ordenAlfabetico)
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <aside className="lg:w-1/4 bg-white dark:bg-white p-6 rounded-xl border border-gray-200 dark:border-gray-200 shadow-lg h-fit text-gray-800 dark:text-gray-800">
          ...
          <form onSubmit={agregarPersonal} className="flex flex-col gap-3">
            <div>
              <label className="text-xs font-bold text-purple-600 dark:text-purple-600 uppercase">
                Cédula
              </label>
              <input
                type="number"
                ...
                className="w-full mt-1 p-2 border border-gray-300 dark:border-gray-300 bg-white dark:bg-white rounded-lg text-sm text-gray-800 dark:text-gray-800 focus:outline-none focus:border-purple-500"
              />
            </div>
            <div>
              <label className="text-xs font-bold text-purple-600 dark:text-purple-600 uppercase">
            ...
            <button
              type="submit"
              className="bg-purple-700 hover:bg-purple-800 text-white font-bold py-2 px-4 rounded-lg text-sm transition-all shadow-md mt-2"
            >
          ...
          <header className="mb-6 border-b border-gray-200 dark:border-gray-200 pb-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
              <div className="min-h-[50px] flex flex-col justify-center">
                <p className="text-sm text-purple-600 dark:text-purple-600 font-bold uppercase tracking-widest">
                  Módulo de Recursos Humanos
                </p>
                <h1 className="text-2xl lg:text-3xl font-black text-gray-800 dark:text-white">
                  {vistaActual === "asistencia"
                    ? `Control de Asistencia — ${semanaSeleccionada}`
                    : "Administración y Estados del Personal"}
                </h1>
              </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
              <div className="flex flex-wrap gap-2 text-sm">
                ...
                <Link
                  to="/"
                  className="bg-white dark:bg-white border border-gray-300 dark:border-gray-300 text-gray-700 dark:text-gray-700 px-4 py-2 rounded-md font-bold hover:bg-gray-50 dark:hover:bg-gray-50 flex items-center shadow-sm"
                >
                  🏠 Inicio
                </Link>
              </div>
              ...
              <button
                ...
                className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-md font-bold shadow-md text-sm"
              >
                💾 Guardar Reporte
              </button>
            </div>
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            <div className="flex bg-white dark:bg-white p-1 rounded-xl font-bold shadow-sm border border-gray-200 dark:border-gray-200">
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-sm transition-all ${turnoSeleccionado === "Mañana" ? "bg-purple-700 text-white shadow-md" : "text-gray-600 dark:text-gray-600 hover:text-gray-900"}`}
              >
                ☀️ Mañana
              </button>
              <button
                onClick={() => setTurnoSeleccionado("Tarde")}
                className={`px-4 py-1.5 rounded-lg text-sm transition-all ${turnoSeleccionado === "Tarde" ? "bg-purple-700 text-white shadow-md" : "text-gray-600 dark:text-gray-600 hover:text-gray-900"}`}
              >
                🌙 Tarde
              </button>
            </div>
              ...
              <>
                <div className="flex bg-white dark:bg-white p-1 rounded-xl font-bold border border-gray-200 dark:border-gray-200">
                  <select
                    ...
                    className="bg-transparent text-sm text-gray-800 dark:text-gray-800 px-3 py-1 focus:outline-none font-bold cursor-pointer"
                  >
                    {meses.map((m) => (
                ...
                <div className="flex bg-white dark:bg-white p-1 rounded-xl font-bold border border-gray-200 dark:border-gray-200 overflow-x-auto">
                  {semanas.map((semana) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-xs transition-all ${semanaSeleccionada === semana ? "bg-purple-600 text-white shadow-md" : "text-gray-600 dark:text-gray-600 hover:text-purple-700"}`}
                    >
                      {semana}
                    </button>
                  ))}
          ...
          {vistaActual === "asistencia" && (
            <div className="bg-white dark:bg-white rounded-xl shadow-2xl border border-gray-200 dark:border-gray-200 overflow-hidden text-gray-800 dark:text-gray-800">
              <div className="bg-white dark:bg-white p-3 border-b border-gray-200 dark:border-gray-200 text-xs font-bold text-purple-800 dark:text-purple-800 uppercase tracking-wider flex justify-between">
                <span>
                  Ingreso de Horas Trabajadas (Meta Semanal Fija:{" "}
                  {META_HORAS_SEMANALES} Horas)
                </span>
                  ...
                  <thead className="bg-white dark:bg-white text-gray-500 dark:text-gray-500 text-xs uppercase border-b border-gray-200 dark:border-gray-200">
                    <tr>
                      ...
                      {diasSemana.map((dia) => (
                        <th
                          key={dia}
                          className="p-3 text-center text-purple-900 dark:text-purple-900 font-black w-[12%]"
                        >
                          {dia} (Hrs)
                        </th>
                      ))}
                      <th className="p-4 text-center bg-white dark:bg-white text-purple-800 dark:text-purple-800 w-[15%]">
                        Total Semanal
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {personalAProcesar.length === 0 ? (
                      <tr>
                        ...
                        >
                          No hay personal activo registrado para el turno{" "}
                          {turnoSeleccionado}.
                        </td>
                      </tr>
                    ) : (
                      personalAProcesar.map((empleado) => {
                        ...
                        const porcentaje = Math.min(
                          (horasAcumuladas / META_HORAS_SEMANALES) * 100,
                          100
                        );
                        ...
                        return (
                          <tr
                            key={empleado.cedula}
                            className="border-t border-gray-200 dark:border-gray-200 hover:bg-purple-50/30 transition-colors"
                          >
                            <td className="p-4 text-left">
                              ...
                              <div className="text-xs text-gray-500 dark:text-gray-500">
                                V-{empleado.cedula} | {empleado.cargo}
                              </div>
                            </td>
                            ...
                            {diasSemana.map((dia) => (
                              <td
                                key={dia}
                                className="p-2 text-center border-x border-gray-100 dark:border-gray-100"
                              >
                                <input
                                  type="number"
                                  min="0"
                            ...
                            <td
                              className={`p-4 font-black text-center border-l border-gray-200 dark:border-gray-200 transition-colors ${cumpleMeta ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}
                            >
                              <div className="text-xl font-extrabold">
                                {horasAcumuladas} h
                              </div>
                              <div className="text-[10px] uppercase tracking-wider font-bold mt-1">
                                de {META_HORAS_SEMANALES} h meta
                              </div>
          ...
          {vistaActual === "administracion" && (
            <div className="bg-white dark:bg-white rounded-xl shadow-2xl border border-gray-200 dark:border-gray-200 overflow-hidden text-gray-800 dark:text-gray-800">
              <div className="bg-white p-4 border-b border-gray-200 text-xs font-bold text-gray-800 uppercase tracking-wider flex justify-between items-center">
                <span>
                  Gestión de Personal Registrado — Historial y Estados
                </span>
                <span className="text-gray-500 font-normal">
                  Total en base de datos: {personalAProcesar.length}
                </span>
              </div>
              <div className="overflow-x-auto">
                  ...
                  <tbody>
                    {personalAProcesar.length === 0 ? (
                      ...
                      personalAProcesar.map((empleado) => (
                        <tr
                          key={empleado.cedula}
                          className="border-t border-gray-200 dark:border-gray-200 hover:bg-purple-50/30 transition-colors text-sm"
                        >
                          <td className="p-4 font-bold text-gray-700 dark:text-gray-700">
                            V-{empleado.cedula}
                          </td>
                          <td className="p-4 font-extrabold text-gray-900 dark:text-gray-900">
                            | {empleado.nombre}
                          </td>
                          <td className="p-4">
                            ...
                            <span className="ml-2 text-xs bg-purple-100 dark:bg-purple-100 text-purple-800 dark:text-purple-800 px-2 py-0.5 rounded font-bold">
                              {empleado.turno}
                            </span>
                          </td>
